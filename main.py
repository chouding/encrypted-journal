...
class Body(tk.Frame):
    ...
    def set_posts(self, posts:list):
        ...
        for post_obj in posts:
            self._insert_post_tree(id, post_obj)
            id += 1
...
class Footer(tk.Frame):
    ...
    """
    Disables the 'Save post' button until user has created or loaded a DSU file.
    """
    ...
    """
    def _draw(self):
        self.save_button = tk.Button(master=self, text="Save Post", width=20)
        self.save_button.configure(command=self.save_click)
...
class MainApp(tk.Frame):
    ...
    def new_profile(self):
        ...
        try:
            ...
            self.footer.save_button["state"] = "normal"
            self.body.reset_ui()
    ...
    def save_profile(self):
        if self.body.get_text_entry() != '':
            ...
            decrypted_post = copy.deepcopy(post)
            self.body.insert_post(decrypted_post)
            self._current_profile.add_post(post)
            self._current_profile.save_profile(self._profile_filename)
            self.body.set_text_entry("")
    
            if self._is_online is True:
                self.publish(decrypted_post)

    """
    Publishes a post to the ICS 32 Distributive Server.
    """
    def publish(self, post:Post):
        req = ds_client.send(SERVER, PORT, self._current_profile.username, self._current_profile.password, post.entry)
        
        if req is True:
            self.footer.set_status('Post successfully sent to DS server')
        else:
            self.footer.set_status('Post was unable to be sent to DS server')
    ...
    def settings_window(self):
        ...
        settings.pack(padx=10, pady=10, fill='x', expand=True)

        self.settings_window = Toplevel(main)
        self.settings_window.geometry('250x175')
        text = tk.Text(self.settings_window, height=2, width=30)
        text.pack()
        if self._current_profile.username == None and self._current_profile.password == None:
            text.insert(tk.END, f'Username:\nPassword:\n')
        else:
            text.insert(tk.END, f'Username: {self._current_profile.username}\nPassword: {self._current_profile.password}\n')
        text.config(state=DISABLED)

        self.new_username = tk.StringVar()
        self.new_password = tk.StringVar()

        usrname_label = ttk.Label(self.settings_window, text='Enter new username:')
        usrname_label.pack()
        usrname_entry = ttk.Entry(self.settings_window, textvariable=self.new_username)
        usrname_entry.pack()

        pwd_label = ttk.Label(self.settings_window, text='Enter new password:')
        pwd_label.pack()
        ...
        self.save_button_settings.pack(ipady=5, pady=10)
